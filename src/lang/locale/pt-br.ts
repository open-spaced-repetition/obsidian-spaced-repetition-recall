...
export default {
    ...
    FLASHCARD_TAGS_DESC: 'Insira etiquetas separadas por espaços ou quebras de linha ex: #flashcards #baralho2 #baralho3.',
    ...
    INLINE_SCHEDULING_COMMENTS: 'Salvar comentários de agendamento na mesma linha que a última linha do flashcard?',
    INLINE_SCHEDULING_COMMENTS_DESC: 'Ligar isso vai fazer com que os comentários em HTML não quebrem a formatação de listas.',
    BURY_SIBLINGS_TILL_NEXT_DAY: 'Enterrar cartas irmãs até o próximo dia?',
    BURY_SIBLINGS_TILL_NEXT_DAY_DESC: 'Cartas irmãs são geradas pelo texto da mesma carta ex: omissão de palavras',
    ...
    CARD_MODAL_SIZE_PERCENT_DESC: 'Deveria estar configurado em 100% em dispositivos móveis ou se você tem imagens muito grandes',
    ...
    CONVERT_HIGHLIGHTS_TO_CLOZES_DESC: 'Adiciona/remove o <code>${defaultPattern}</code> dos seus "Padrões de Omissão"',
    CONVERT_BOLD_TEXT_TO_CLOZES: 'Converter **texto em negrito** em omissões?',
    CONVERT_BOLD_TEXT_TO_CLOZES_DESC: 'Adiciona/remove o <code>${defaultPattern}</code> dos seus "Padrões de Omissão"',
    CONVERT_CURLY_BRACKETS_TO_CLOZES: 'Converter {{chaves}} em omissões?',
    CONVERT_CURLY_BRACKETS_TO_CLOZES_DESC: 'Adiciona/remove o <code>${defaultPattern}</code> dos seus "Padrões de Omissão"',
    ...
    FIX_SEPARATORS_MANUALLY_WARNING: 'Note que depois de mudar isso você vai ter que manualmente mudar quaisquer flashcards que você tenha.',
    ...
    ENABLE_FILE_MENU_REVIEW_OPTIONS: 'Ative as opções de revisão no menu Arquivo (ex.: Revisão: Fácil, OK, Difícil)',
    ENABLE_FILE_MENU_REVIEW_OPTIONS_DESC: 'Se você desativar as opções de revisão no menu Arquivo, poderá revisar suas anotações usando os comandos do plugin e, se os tiver definido, as teclas de atalho associadas.',
    INITIALLY_EXPAND_SUBDECKS_IN_TREE: 'Árvores de baralhos devem inicialmente ser exibidas como expandidas',
    INITIALLY_EXPAND_SUBDECKS_IN_TREE_DESC: 'Desabilite isso para colapsar baralhos que estão um dentro do outro na mesma carta. Útil se você tem cartas que pertencem a muitos baralhos em um mesmo arquivo.',
    ALGORITHM: 'Algorítmo',
    CHECK_ALGORITHM_WIKI: 'Para mais informações, cheque a <a href="${algoUrl}">implementação do algorítmo</a>.',
    ...
    EASY_BONUS_DESC: 'O bônus de fácil te permite mudar a diferência entre intervalos de responder OK e Fácil em um(a) flashcard/nota (mínimo = 100%).',
    ...
    MAX_LINK_CONTRIB_DESC: 'Contribuição máxima da facilidade ponderada das notas linkadas à facilidade inicial.',
    FUZZING: 'Fuzzing',
