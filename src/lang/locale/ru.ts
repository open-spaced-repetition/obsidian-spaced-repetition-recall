...
export default {
    ...
    GITHUB_DISCUSSIONS: 'Посмотрите <a href="${discussionsUrl}">Обсуждения</a> для помощи, отзывов и общих дискуссий.',
    GITHUB_ISSUES: 'Сообщите о проблеме <a href="${issuesUrl}">здесь</a>, если у вас есть запрос на функцию или сообщение об ошибке.',
    GITHUB_ISSUES_MODIFIED_PLUGIN: 'Сообщите о проблеме <a href="${issuesUrl}">здесь</a> касательно этого модифицированного sr-плагина, если у вас есть запрос на функцию или сообщение об ошибке.',
    ...
    TRANSLATION_CONTRIBUTION_INFO: 'Перевести плагин на <a href="${translationContributionUrl}">другой язык</a>',
    FOLDERS_TO_IGNORE: 'Игнорируемые папки',
    FOLDERS_TO_IGNORE_DESC: 'Введите пути к папкам или глобальным шаблонам на отдельных строках, например: Templates/Scripts или **/*.excalidraw.md. Эта настройка общая для карточек и заметок.',
    ...
    FLASHCARD_TAGS_DESC: 'Введите теги, разделённые пробелами или новыми строками, например: #карточки #колода2 #колода3.',
    ...
    BURY_SIBLINGS_TILL_NEXT_DAY_DESC: 'Родственные карточки - те, которые образованы из одного текста, пример: закрытые карточки ([...])',
    ...
    SHOW_INTERVAL_IN_REVIEW_BUTTONS_DESC: 'Полезно знать, на какое время в будущем откладываются ваши карточки.',
    CARD_MODAL_HEIGHT_PERCENT: 'Высота карточки в процентах',
    CARD_MODAL_SIZE_PERCENT_DESC: 'Установите 100% на мобильных устройствах, если нужно просматривать большие изображения',
    ...
    REVIEW_DECK_ORDER_PREV_DECK_COMPLETE_SEQUENTIAL: 'Последовательно (после завершения предыдущей колоды)',
    ...
    CONVERT_HIGHLIGHTS_TO_CLOZES_DESC: 'Добавить/удалить <code>${defaultPattern}</code> в ваши "Шаблоны закрытых карточек"',
    CONVERT_BOLD_TEXT_TO_CLOZES: 'Конвертировать **жирный текст** в закрытые карточки (пример: [...])?',
    CONVERT_BOLD_TEXT_TO_CLOZES_DESC: 'Добавить/удалить <code>${defaultPattern}</code> в ваши "Шаблоны закрытых карточек"',
    CONVERT_CURLY_BRACKETS_TO_CLOZES: 'Конвертировать {{фигурные скобки}} в закрытые карточки (пример: [...])?',
    CONVERT_CURLY_BRACKETS_TO_CLOZES_DESC: 'Добавить/удалить <code>${defaultPattern}</code> в ваши "Шаблоны закрытых карточек"',
    CLOZE_PATTERNS: 'Шаблоны закрытых карточек',
    CLOZE_PATTERNS_DESC: 'Введите шаблоны закрытых карточек, разделенные переводами строк. Посмотрите <a href="${docsUrl}">документацию</a> для ознакомления.',
    INLINE_CARDS_SEPARATOR: 'Разделитель для внутристрочных карточек',
    FIX_SEPARATORS_MANUALLY_WARNING: 'Внимание: после изменения этого параметра вам потребуется вручную изменить разделители в существующих карточках.',
    ...
    ENABLE_FILE_MENU_REVIEW_OPTIONS: 'Включите параметры обзора в меню Файл (т.е.: Изучение: Легко, Помню, Сложно)',
    ...
    INITIALLY_EXPAND_SUBDECKS_IN_TREE_DESC: 'Отключите этот параметр, чтобы свернуть вложенные колоды в одной карточке. Полезно, если у вас есть карточки, принадлежащие многим колодам в одном файле.',
    ALGORITHM: 'Алгоритм',
    ...
    MAX_INTERVAL_DESC: 'Устанавливает верхний предел интервала повторения (по умолчанию = 100 лет).',
    MAX_INTERVAL_MIN_WARNING: 'Максимальный интервал должен быть не менее 1 дня.',
    MAX_LINK_CONTRIB: 'Максимальный вклад ссылок',
    MAX_LINK_CONTRIB_DESC: 'Максимальный вклад взвешенной "Лёгкости" связанных заметок в исходную "Лёгкость".',
    FUZZING: 'Размытие',
    FUZZING_DESC: 'При включении добавляет небольшую случайную задержку к новому времени интервала, чтобы предотвратить скопление карточек и их повторение в один день.',
    SWITCH_SHORT_TERM: 'Переключиться на краткосрочное планирование',
    SWITCH_SHORT_TERM_DESC: 'При отключении позволяет пользователю пропустить краткосрочное планирование и сразу перейти к долгосрочному.',
    ...
    ALGORITHMS_CONFIRM: 'Переключение алгоритмов может сбросить или повлиять на время повторения существующих элементов. Это изменение необратимо. Изменение алгоритмов вступает в силу только после перезапуска или перезагрузки плагина. Вы уверены, что хотите переключить алгоритм?',
    ...
    REVIEW_FLOATBAR_DESC: 'Работает только при включённом автоматическом переходе к следующей заметке. Показывается при повторении заметки через клик по строке состояния/боковой панели/команде.',
    REVIEW_NOTE_DIRECTLY: 'Повторять заметку напрямую?',
    REVIEW_NOTE_DIRECTLY_DESC: 'При повторении заметки через клик по строке состояния или команде открывать её напрямую, без необходимости выбирать тег',
    ...
    REQUEST_RETENTION_DESC: 'Вероятность (в процентах), что вы ожидаете вспомнить ответ при следующем повторении',
    REVLOG_TAGS: 'Теги для вывода в журнал повторений',
    REVLOG_TAGS_DESC: 'Теги для вывода в журнал повторений, могут быть тегами карточек или/и заметок. По умолчанию пусто - все выводятся в журнал без разделения по тегам',
